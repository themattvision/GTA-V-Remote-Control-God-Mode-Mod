...
Il setup e il menu nell'area di notifica usano automaticamente l'italiano quando Windows è impostato in italiano, altrimenti l'inglese.

## Installazione

1. Scarica `Windows-GodMode-Mod-Remote-Control-Setup-v0.3.1.exe` dalla release GitHub.
...
Se Windows SmartScreen segnala l'eseguibile, il motivo è che il build corrente non è firmato. Non disattivare SmartScreen a livello globale.

## Uso sicuro

Il bridge invia tasti soltanto quando il processo in primo piano è `GTA5.exe` e l'eseguibile appartiene alla cartella autorizzata. Supporta soltanto i comandi compilati nell'app e non accetta tasti, testo, comandi shell o native hash arbitrari dalla rete.
...
1. Avvia GTA V Legacy ed entra in modalità Storia.
...
Per il test completo usa [`docs/e2e-checklist.it.md`](../docs/e2e-checklist.it.md).
...
Lo script esegue i test, pubblica l'app self-contained, compila `GTARemoteBridge.asi` e crea `Windows-GodMode-Mod-Remote-Control-Setup-v0.3.1.exe` nella cartella Download dell'utente.
